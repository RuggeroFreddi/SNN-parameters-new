...
BASE_DIR = "compare/statistic_accuracy_reset/"
...
def prepare_sel(df, k, order):
    ...
    if not df_sel.empty:
        ...
        # normalizza anche le x in [0,1]
        df_sel["weight_norm"] = normalize_0_1(df_sel["weight"])
    return df_sel
...
plt.figure()
if not df_beta_1.empty:
    plt.plot(df_beta_1["weight_norm"], df_beta_1["accuracy_rf_smooth"],
             marker="o", label="beta (min param)")
if not df_curr_1.empty:
    plt.plot(df_curr_1["weight_norm"], df_curr_1["accuracy_rf_smooth"],
             marker="s", label="current_amp (min param)")
if not df_mem_1.empty:
    plt.plot(df_mem_1["weight_norm"], df_mem_1["accuracy_rf_smooth"],
             marker="^", label="membrane (max param)")

plt.xlabel("weight (norm 0–1)")
...
plt.figure()
if not df_beta_2.empty:
    plt.plot(df_beta_2["weight_norm"], df_beta_2["accuracy_rf_smooth"],
             marker="o", label="beta (2° param)")
if not df_curr_2.empty:
    plt.plot(df_curr_2["weight_norm"], df_curr_2["accuracy_rf_smooth"],
             marker="s", label="current_amp (2° param)")
if not df_mem_2.empty:
    plt.plot(df_mem_2["weight_norm"], df_mem_2["accuracy_rf_smooth"],
...
plt.figure()
if not df_beta_3.empty:
    plt.plot(df_beta_3["weight_norm"], df_beta_3["accuracy_rf_smooth"],
             marker="o", label="beta (max param)")
if not df_curr_3.empty:
    plt.plot(df_curr_3["weight_norm"], df_curr_3["accuracy_rf_smooth"],
             marker="s", label="current_amp (max param)")
if not df_mem_3.empty:
    plt.plot(df_mem_3["weight_norm"], df_mem_3["accuracy_rf_smooth"],
...
if "accuracy_slp_smooth" in df_beta_1:
    plt.plot(df_beta_1["weight_norm"], df_beta_1["accuracy_slp_smooth"],
             marker="o", label="beta (min param)")
if "accuracy_slp_smooth" in df_curr_1:
    plt.plot(df_curr_1["weight_norm"], df_curr_1["accuracy_slp_smooth"],
             marker="s", label="current_amp (min param)")
if "accuracy_slp_smooth" in df_mem_1:
    plt.plot(df_mem_1["weight_norm"], df_mem_1["accuracy_slp_smooth"],
             marker="^", label="membrane (max param)")
...
if "accuracy_slp_smooth" in df_beta_2:
    plt.plot(df_beta_2["weight_norm"], df_beta_2["accuracy_slp_smooth"],
             marker="o", label="beta (2° param)")
if "accuracy_slp_smooth" in df_curr_2:
    plt.plot(df_curr_2["weight_norm"], df_curr_2["accuracy_slp_smooth"],
             marker="s", label="current_amp (2° param)")
if "accuracy_slp_smooth" in df_mem_2:
    plt.plot(df_mem_2["weight_norm"], df_mem_2["accuracy_slp_smooth"],
             marker="^", label="membrane (2° param)")
...
if "accuracy_slp_smooth" in df_beta_3:
    plt.plot(df_beta_3["weight_norm"], df_beta_3["accuracy_slp_smooth"],
             marker="o", label="beta (max param)")
if "accuracy_slp_smooth" in df_curr_3:
    plt.plot(df_curr_3["weight_norm"], df_curr_3["accuracy_slp_smooth"],
             marker="s", label="current_amp (max param)")
if "accuracy_slp_smooth" in df_mem_3:
    plt.plot(df_mem_3["weight_norm"], df_mem_3["accuracy_slp_smooth"],
             marker="^", label="membrane (min param)")
