...
class Reservoir:
    ...
    def __init__(self, simulation_params: SimulationParams, stdp_params: Optional[STDPParams] = None) -> None:
        ...
        self.mean_distance = simulation_params.mean_distance
        ...
        self.leak_cv: float = simulation_params.leak_cv
        ...
        # leak per-neurone: lognormale con mean = leak_coefficient, std = mean/10
        ...
        if m > 0.0:
            ...
            # caso limite: niente lognormale se m <= 0
            self.leak_vector = np.full(
                self.num_neurons, m, dtype=np.float32
            )

        # fattore di leak = 1 - coefficiente di leak
        self.leak_factor_vector = 1.0 - self.leak_vector
        self.leak_factor = np.float32(1.0 - self.leak_coefficient)
    ...
    def _generate_synaptic_weights_small_world(
        ...
        """Genera pesi sinaptici con grafo small-world e una sola popolazione di neuroni.

        - Topologia: grafo small-world di Watts–Strogatz.
        - Tutti i neuroni sono dello stesso tipo.
        - I pesi sono estratti da UNA sola distribuzione lognormale:
            * media (dei pesi) = self.weights_mean
            * deviazione standard (dei pesi) = self.weights_std
        """
        ...
        # Orienta gli archi (pre -> post) in modo casuale
        flip = np.random.rand(edges.shape[0]) < 0.5
        rows = np.where(flip, edges[:, 0], edges[:, 1])  # presinaptici
        cols = np.where(flip, edges[:, 1], edges[:, 0])  # postsinaptici
        n_edges = rows.shape[0]

        # Parametri globali: media e deviazione standard dei pesi
        mean_total = float(self.weights_mean)
        var = self.weights_variance
        
        if mean_total <= 0 or var <= 0:
            raise ValueError(
                "Per pesi lognormali servono mean_total > 0 e std > 0 "
                "(tutti i neuroni sono eccitatori in questo modello)."
            )

        

        # Conversione mean/std (dei pesi) -> parametri (mu, sigma) della lognormale
        # Se X ~ LogNormal(mu, sigma), allora:
        #   E[X] = exp(mu + sigma^2 / 2)
        #   Var[X] = (exp(sigma^2) - 1) * exp(2*mu + sigma^2)
        sigma2 = np.log(1.0 + var / (mean_total ** 2))
        sigma = np.sqrt(sigma2)
        mu = np.log(mean_total) - 0.5 * sigma2

        # Estrai tutti i pesi dalla stessa distribuzione lognormale
    ...
    def _generate_synaptic_weights_small_world_old(
        self,
        small_world_graph_p: float = 0.1,
        small_world_graph_k: int = 10,
    ) -> None:
        ...
        - 80% neuroni eccitatori, 20% inibitori (assegnati a caso).
        - Eccitatori: +lognormal, inibitori: -lognormal.
        - Varianza dei pesi uguale per E e I.
        - Media totale pesi = self.weights_mean.
        - Distanza tra medie E/I = mean_distance.
        """
        # Grafo small-world
        small_world_graph = nx.watts_strogatz_graph(
            n=self.num_neurons,
        ...
        # ----------------------------------------------------------------------
        # 1) Assegna 80% neuroni eccitatori e 20% inibitori, distribuiti a caso
        # ----------------------------------------------------------------------
        Ne = int(round(0.8 * self.num_neurons))     # 80% eccitatori
        Ne = max(1, min(Ne, self.num_neurons - 1))  # evita casi degeneri
        Ni = self.num_neurons - Ne                  # 20% inibitori

        neuron_is_excitatory = np.zeros(self.num_neurons, dtype=bool)
        exc_indices = np.random.choice(self.num_neurons, size=Ne, replace=False)
        neuron_is_excitatory[exc_indices] = True

        # Tipo di ogni sinapsi determinato dal neurone presinaptico
        edge_is_excitatory = neuron_is_excitatory[rows]
        edge_is_inhibitory = ~edge_is_excitatory

        n_e = int(edge_is_excitatory.sum())
        n_i = int(edge_is_inhibitory.sum())

        # Parametri globali di media/varianza (come nel codice originale)
        mean_total = self.weights_mean
        std = abs(self.weights_mean) * self.weights_variance
        var = std ** 2
        
        if mean_total <= 0 or var <= 0:
            raise ValueError("Lognormal weights require positive mean and variance")

        # Se il grafo (per struttura) non dà archi di uno dei due tipi, fallback
        if n_e == 0 or n_i == 0:
            sigma2 = np.log(1.0 + var / (mean_total ** 2))
            sigma = np.sqrt(sigma2)
            mu = np.log(mean_total) - 0.5 * sigma2
            ...
            # ------------------------------------------------------------------
            # 2) Medie E/I con vincoli:
            #    0.8 * mean_e + 0.2 * mean_i = mean_total
            #    mean_e - mean_i = mean_distance
            #
            #    mean_e > 0 (eccitatori)
            #    mean_i < 0 (inibitori, pesi negativi)
            #
            # Sviluppo:
            #   mean_i = mean_e - mean_distance
            #   0.8*mean_e + 0.2*(mean_e - mean_distance) = mean_total
            #   (0.8 + 0.2)*mean_e - 0.2*mean_distance = mean_total
            #   mean_e = mean_total + 0.2 * mean_distance
            #   mean_i = mean_e - mean_distance = mean_total - 0.8 * mean_distance
            # ------------------------------------------------------------------
            fe = 0.8
            fi = 0.2
            d = self.mean_distance

            mean_e = mean_total + fi * d          # media pesi eccitatori (positiva)
            mean_i = mean_total - fe * d          # media pesi inibitori (negativa attesa)

            # Per la lognormale inibitoria usiamo la media in modulo
            mean_i_mag = -mean_i                  # > 0 se mean_i < 0

            if mean_e <= 0 or mean_i_mag <= 0:
                raise ValueError(
                    "Mean distance too large or incompatible: "
                    "require mean_e > 0 and inhibitory mean < 0."
                )

            weights = np.empty(n_edges, dtype=np.float32)

            # ------------------------------------------------------------------
            # 3) Lognormal per ECCITATORI (media = mean_e, var = var)
            # ------------------------------------------------------------------
            sigma2_e = np.log(1.0 + var / (mean_e ** 2))
            sigma_e = np.sqrt(sigma2_e)
            mu_e = np.log(mean_e) - 0.5 * sigma2_e

            weights[edge_is_excitatory] = np.random.lognormal(
                mean=mu_e,
                sigma=sigma_e,
                size=n_e,
            ).astype(np.float32, copy=False)
            ...
            # ------------------------------------------------------------------
            # 4) Lognormal per INIBITORI: estrai modulo positivo, poi metti il segno -
            #     magnitudine ~ LogNormal(mean_i_mag, var)
            #     peso = -magnitudine
            # ------------------------------------------------------------------
            sigma2_i = np.log(1.0 + var / (mean_i_mag ** 2))
            sigma_i = np.sqrt(sigma2_i)
            mu_i = np.log(mean_i_mag) - 0.5 * sigma2_i

            inh_magnitudes = np.random.lognormal(
                mean=mu_i,
                sigma=sigma_i,
                size=n_i,
            ).astype(np.float32, copy=False)

            weights[edge_is_inhibitory] = -inh_magnitudes

        # ----------------------------------------------------------------------
        # 5) Costruisci matrice sparsa come nell'implementazione originale
        # ----------------------------------------------------------------------
        self.synaptic_weights = csr_matrix(
            (weights, (rows, cols)), shape=(self.num_neurons, self.num_neurons)
        )
